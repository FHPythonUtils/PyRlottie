...
async def convSingleLottie(  # pylint: disable=too-many-arguments
	...
	cmd = (
		f"{THISDIR}/lottie2gif {lottieFile.path} {_scale(lottieFile.data['w'], scale)}"
	...
	if len(webpFiles):
		cmd = f"{THISDIR}/gif2webp {gifFiles[0]} -o {webpFiles[0]} -mt"
		ret = await _execSubprocess(cmd)
		if ret[0] > 0:
			msg = f"{cmd=}: {ret=}"
			raise RuntimeError(msg)
